...
import Repo from "../../../assets/repogit.png";
      ...
      <div className="border min-w-[680px] w-[fit-content] min-h-1/2 h-[650px] mx-auto bg-gray-800 border-4 border-blue-700 rounded-md shadow-md border">
        ...
        <div className="flex flex-row justify-start bg-gray-200 items-center text-black text-sm">
          ...
          <div className="flex border-2 border-gray-300 items-center">
            ...
            <a href={url} target="_blank" className="flex mr-[30px] text-black cursor-pointer hover:bg-blue-200 ml-3 items-center"><img src={OpenNew} className="size-7" />{language === "en" ? "Open in Browser" : "Abrir en tu navegador"}</a>
          </div>
        </div>
        {/* Barra de navegacion */}
        <div className="flex flex-row justify-start bg-gray-200 items-center text-black text-sm">
          ...
          {url?.includes("github") && !url?.includes("Pinball") &&
            <div className="mt-10">
              <p> {language === "es"
                ...
                : "This is a GitHub page. You can't view it here. Click the button below to redirect to GitHub."}</p>
              <img src={Repo} className="size-36 mx-auto mt-5 cursor-pointer animate-pulse" alt="Open in github button" onClick={() => window.open(url)} />
            </div>
          }
